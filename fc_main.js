...
function setOverrides() {
  ...
  FrozenCookies.frenzyTimes =
    JSON.parse(localStorage.getItem('frenzyTimes')) || {};
  ...
  if (M.magic == M.magicM) {
    switch (FrozenCookies.autoSpell) {
      ...
      case 3:
        ...
        if (
          Game.Objects['Fractal engine'].amount == 0 ||
        ...
        while (
          Game.Objects['Fractal engine'].amount >= 400 ||
          Game.cookies < Game.Objects['Fractal engine'].price / 2
        ) {
          ...
          logEvent(
            'Store',
            'Sold 1 Fractal engine for ' +
              Beautify(Game.Objects['Fractal engine'].price * 1.15 * 0.5)
          );
        }
        M.castSpell(SE);
  ...
  switch (FrozenCookies.blacklist) {
    ...
    case 3:
      FrozenCookies.blacklist =
        haveAll('halloween') && haveAll('valentines') ? 5 : 3;
      break;
  }
}
...
function recommendationList(recalculate) {
  if (recalculate) {
    ...
    if (
      ...
      Game.Objects['Fractal engine'].amount >= 399
    ) {
...
function buildingStats(recalculate) {
  if (recalculate) {
    ...
    FrozenCookies.caches.buildings = Game.ObjectsById.map(function(
      ...
      if (
        buildingBlacklist === true ||
        _.contains(buildingBlacklist, current.id)
      ) {
        return null;
      }
